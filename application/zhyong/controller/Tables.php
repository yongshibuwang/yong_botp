...
class Tables extends Father
{
    ...
    public function index()
    {
        ...
        $article=Db::table('article')->where('status=1 AND kind=1')->order('add_time desc')->where($where)->select();
        $count=Db::table('article')->where('status=1 AND kind=1')->where($where)->count();
        $pagesize=10;
        $url=$this->request->domain().'/zhyong/Tables/index';
        if(isset($_GET['page'])){
    ...
    public function tables()
    {
        ...
        if($type) $where['type']=$type;
        $this->assign('type',$type);
        $article=Db::table('article')->where('status=1 AND kind=2')->order('add_time desc')->where($where)->select();
        $count=Db::table('article')->where('status=1 AND kind=2')->where($where)->count();
        $pagesize=10;
        $url=$this->request->domain().'/zhyong/Tables/tables';
